...
def string_date_to_date(date):
    ...
    if final_date[0] == '-' or final_date[0] == 'N/A':
        return None
...
if __name__ == "__main__":
    ...
    for league_index in range(constants_transfermarkt.LEAGUE_INDEX, len(constants_transfermarkt.LEAGUES_LINK)):
        ...
        for year in constants_transfermarkt.YEARS:
            ...
            site = '{}/{}/startseite/wettbewerb/{}/plus/?saison_id={}'.format(constants_transfermarkt.TRANSFERMARKT_URL,
                                                                              constants_transfermarkt.LEAGUES_LINK[league_index],
                                                                              constants_transfermarkt.LEAGUES_SHORT[league_index],
                                                                              year)
            print(site)
            browser.get(site)
            ...
            league = soup.find('h1', {'class': 'spielername-profil'}).text
            for link in list_array:
                team_link = '{}{}'.format(constants_transfermarkt.TRANSFERMARKT_URL, link)
                browser.get(team_link)
                soup = BeautifulSoup(browser.page_source, "html.parser")
                team = soup.find('div', {'class': 'dataMain'}).find('h1', {'itemprop': 'name'}).find('span').text
                print(team)
                directory = 'data/%s/%s/' % (
                    constants_transfermarkt.LEAGUES_LINK[league_index], year)
                pathlib.Path(directory).mkdir(parents=True, exist_ok=True)
                players_file_name = directory + '%s- %s - Players-%s.json' % (
                    team, constants_transfermarkt.LEAGUES_LINK[league_index], year)
                ...
                for idx, players in enumerate(player_team):
                    ...
                    new_player_preview.league = league
