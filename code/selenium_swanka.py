...
for num_season in range(len(constants.years)):
    ...
    site = constants.url + season
    browser.get(site)
    req = requests.get(constants.url + season, headers=constants.hdr)
    page = req.text
    ...
    for team in teams:
        ...
        link = team.get('href')
        new_link = link + '#performance-score#spanish-la-liga#season-' + season.replace('-', '/') + "#" + \
                   constants.values_num[
                       num_season] + "#all-matches#1-38#by-match"
            ...
            if (can_Load):
                ...
                if (position != 'Goalkeeper'):
                    ...
                    if (player_sw.app_tot > 0):
                        ...
                        if (player_sw.total_def_err > 0):
                            ...
                            while (not_fail):
                                try:
                                    ...
                                    def_err_lt = soup_defensive_error.find('div', {'id': 'the-graph-11'}).find('div', {
                                        'aria-label': constants.arial_label}).find('tbody').findAll('td')
                        ...
                        write = write + str(player_sw.pass_forward) + ',' + str(player_sw.pass_backward) + ',' + str(
                        ...
                        write = write + str(player_sw.unsucc_take_on) + ',' + str(player_sw.succ_head_duel) + ',' + str(
                        ...
                        write = write + str(player_sw.tack_yel_card) + ',' + str(player_sw.verb_yel_card) + ',' + str(
                            player_sw.oth_yel_card) + ',' + str(player_sw.tot_red_card) + ','
                        write = write + str(player_sw.viol_red_card) + ',' + str(player_sw.tack_red_card) + ',' + str(
                    ...
                    if (goalkeeper_sw.app_tot > 0):
                        ...
                        write = write + str(goalkeeper_sw.clean_sheets) + ',' + str(
                            goalkeeper_sw.total_goal_conceed) + ',' + str(goalkeeper_sw.avg_goals_conceed) + ',' + str(
                        ...
                        write = write + str(goalkeeper_sw.goals_set_piece_crosses) + ',' + str(
                            ...
                            goalkeeper_sw.goals_open_play) + ',' + str(goalkeeper_sw.goals_open_play_outside_box) + ','
                        ...
                        write = write + str(goalkeeper_sw.avg_saves_per_goal) + ',' + str(
                            goalkeeper_sw.total_saves_goal) + ',' + str(goalkeeper_sw.avg_claims_success) + ',' + str(
